...
### Fixed
- Scoop bucket publish failing on release with 403; GoReleaser now uses
  `HOMEBREW_TAP_GITHUB_TOKEN` for both Homebrew and Scoop (one PAT must cover
  both `homebrew-shadowbox` and `scoop-shadowbox`).
